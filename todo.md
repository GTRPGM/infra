...
- 허용: `scenario-service`, `gm`, `src/tester`, 실행 스크립트/문서/테스트 보강.
...
- 핵심 5개 서비스(gm/state/scenario/rule/llm) 모두 `healthy` 또는 200 응답.

### Phase B. 시나리오 생성→주입 스키마 정렬 (최우선)

1. `state-manager` 주입 규격을 정식 계약으로 고정.

- 기준: `state-manager/src/state_db/schemas/scenario.py`
- 기준: `state-manager/src/state_db/repositories/scenario.py` 실제 저장 로직.

2. `scenario-service` 주입 DTO/변환기 정렬.

- 대상: `scenario-service/src/scenario/core/models/generation.py`
- 대상: `scenario-service/src/scenario/core/engine/scenario_engine.py`
- 대상: `scenario-service/src/scenario/plugins/db/adapter.py`
- 조치:
...
- `scenario-service/tests/test_injection_reference_match.py`를 state 기준으로 업데이트.
...
- 대상: `gm/src/gm/plugins/external/http_client.py`
- 대상: `gm/src/gm/core/engine/game_engine.py`
- 조치:
...
- `gm/src/gm/schemas/*`와 실제 외부 응답 파싱 정렬.
- 회귀 테스트 보강(`gm/tests/test_rule_integration.py` 중심).
...
- `uv run python -m src.tester.runner [session_id] [max_turns]`
- 기본 10턴 + 확장 20턴 검증.
...
- 코드 수정: `scenario-service`, `gm`, `src/tester` 한정
