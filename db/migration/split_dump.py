    ...
    with open(input_file, "r", encoding="utf-8") as f_in:
        for line in f_in:
            ...
            if line.startswith("CREATE TABLE public."):
                ...
                try:
                    table_name = (
                        line.split(" ON public.")[1].split(" ")[0].strip().split("(")[0]
                    )
                    current_target = get_target_db(table_name)
                except Exception:
            ...
            if (
                ...
                or line.startswith("ALTER SEQUENCE")
                or line.startswith("SELECT pg_catalog.setval")
            ):
                is_global = True

            # Types/Domains/Functions: Write to ALL (simplification)
            if (
                line.startswith("CREATE TYPE")
                or line.startswith("CREATE DOMAIN")
