...
## 3.1) NPC/적 턴 대사 분리(post-fix2, 2026-02-09)
- 목표: NPC/적 턴에서 직접 발화를 `dialogue`로 분리해 응답에 포함(행동은 응답 미노출)
- 적용 결과:
  - BE-router 경유 E2E가 시나리오 종료까지 성공
  - NPC 턴에서 `[GM (NPC 대사)]` 로그가 실제로 출력됨
  - 근거 로그: `log/test_output_llm-e2e-end-1770616503_fix7_20260209_151825.log`
- 추가 조치:
  - 시퀀스 전이가 발생하더라도, 전이 이후 시퀀스 기준으로 NPC/적 턴을 1회 수행하도록 변경
    (기존에는 전이 시 NPC 턴이 스킵되어 “적이 아예 행동하지 않는” 문제가 발생)
...
## 4.1) 잔존 전투 정합성 이슈 (2026-02-09)
- 적 턴 서사에서 “플레이어가 큰 피해를 입었다”는 묘사가 나오나, 상태의 플레이어 HP가 감소하지 않는 케이스가 관찰됨.
- 적 턴에서 단일 타겟 공격이어야 하는데, 서사가 “모든 적이 피해를 입는다” 같은 AOE로 확장되는 케이스가 관찰됨.
- 위 두 항목은 룰엔진 diffs/commit 스키마 정합 또는 나레이션 가드레일(상태 기반 피해 묘사 제한) 영역으로 추가 점검 필요.
